	...
	implements OnInit
{
	...
	private childrenList: QueryList<VisualComponent>;
	...
	public setParentNode(
		...
		if (
			...
			this.parentCollection.children.indexOf(this.collection) === -1
		) {
	...
	public getVisual(): HTMLElement {
		if (this.visual === null || this._needUpdate) {
			...
			switch (this.type.toLowerCase()) {
				...
				case 'radio':
					...
					if (
						ThreeUtil.isNotNull(this.radioValues) &&
						this.radioValues.length > 0
					) {
		...
		if (
			...
			this.visual.parentNode !== this.parentNode
		) {
