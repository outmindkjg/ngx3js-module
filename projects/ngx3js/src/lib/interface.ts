...
export class ThreeUtil {
  ...
  public static isColor(color: any): boolean {
    if (color instanceof THREE.Color || typeof color === 'number') {
      return true;
    } else if (typeof color === 'string' && (color.startsWith('0x') || color.startsWith('rgb(') || color.startsWith('RGB(') || color.startsWith('color(') || color.startsWith('COLOR(') || color.startsWith('hsl(') || color.startsWith('HSL(') || color.startsWith('#') || /^[a-z]{3,10}$/.test(color) || color.indexOf('random') >= 0)) {
      return true;
    } else {
      return false;
    }
  ...
  public static getMaterialByType(material: any, materialType?: string): THREE.Material {
    ...
    if (this.isNotNull(materialType) && materialType != '') {
      ...
      if (Array.isArray(matList)) {
        matList.forEach((mat) => {
          if ((this.isNull(mat.userData.materialType) || materialType.toLowerCase() === mat.userData.materialType) && (this.isNull(mat.userData.refName) || mat.userData.refName == '')) {
            matchedMat = mat;
          }
        });
      } else if ((this.isNull(matList.userData.materialType) || materialType.toLowerCase() === matList.userData.materialType) && (this.isNull(matList.userData.refName) || matList.userData.refName == '')) {
        matchedMat = matList;
      }
    } else {
      const matList = this.getMaterial(material);
