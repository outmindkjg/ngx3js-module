...
export class RendererComponent extends AbstractSubscribeComponent implements OnInit, AfterContentInit, AfterViewInit, OnChanges {
  ...
  public applyChanges(changes: string[]) {
    if (this.renderer !== null) {
      ...
      if (!ThreeUtil.isOnlyIndexOf(changes, ['useevent', 'shared', 'width', 'height', 'x', 'y', 'resize', 'scene', 'camera', 'control', 'composer', 'viewer', 'listener', 'audio', 'controller', 'lookat', 'control', 'localclippingenabled', 'globalclippingenabled', 'clearcolor', 'clearalpha', 'tonemapping', 'tonemappingexposure', 'shadowmapenabled', 'physicallycorrectlights', 'shadowmaptype', 'composerenable', 'autoclear', 'autoclearcolor', 'outputencoding', 'clippingplanes', 'canvas2d', 'cssclass', 'controltype', 'controloptions', 'guiparams', 'guicontrol', 'size'], this.OBJECT_ATTR)) {
   ...
   * @param [options]
   */
  public getCanvasJson(callback: (json: any) => void, options?: { width?: number; height?: number; name?: string; type?: string }) {
