...
export class PhysicsConstraintComponent extends AbstractSubscribeComponent implements OnInit {
  ...
  private getBtVector3(x: number, y: number, z: number): Ammo.btVector3 {
    return new this.ammo.btVector3(ThreeUtil.getTypeSafe(x, 0), ThreeUtil.getTypeSafe(y, 0), ThreeUtil.getTypeSafe(z, 0));
  }
  ...
  update(timer: RendererTimer) {
    if (this.constraint !== null) {
      if (this.constraint instanceof this.ammo.btHingeConstraint && this.enableMotor) {
        this.constraint.enableAngularMotor(this.enableMotor, ThreeUtil.getTypeSafe(this.targetVelocity, 0) * timer.delta, ThreeUtil.getTypeSafe(this.maxMotorImpulse, 50));
      }
    }
  }
}
