import { BufferGeometry, Float32BufferAttribute, RingBufferGeometry, Vector2 } from 'three';
 ...
 *
 */
export class NgxRingDepthGeometry extends BufferGeometry {
	/**
	...
	constructor(
		...
		const frontGeometry = new RingBufferGeometry(
			innerRadius,
			outerRadius,
			thetaSegments,
			phiSegments,
		...
		for (let i = 0; i < gridX - 1; i++) {
			const idx = i;
			frontVertices.push(
				frontAttribute.getX(idx),
				frontAttribute.getY(idx),
				frontAttribute.getZ(idx)
			);
			backVertices.push(
				backAttribute.getX(idx),
				backAttribute.getY(idx),
				backAttribute.getZ(idx)
			);
		...
		if (depthRate !== 1) {
			...
			for (let i = 0; i < vertices.length; i += 3) {
				...
				vector2.y = y;
				const rate =
					((vector2.length() - innerRadius) / (outerRadius - innerRadius)) *
					(1 - depthRate);
				vertices[i + 2] += -z * rate;
			}
		}
		...
		if (!isClosed) {
			for (let i = 0; i < sideIdxLen; i++) {
				...
				indices.push(
					startSideSize + i,
					startSideSize + topIdx,
					startSideSize + sideIdxLen + i
				);
				indices.push(
					startSideSize + sideIdxLen + topIdx,
					startSideSize + sideIdxLen + i,
					startSideSize + topIdx
				);
				groupEnd += 6;
