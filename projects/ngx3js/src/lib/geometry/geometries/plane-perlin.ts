 ...
 *
 */
export class NgxPlanePerlinGeometry {
	/**
	...
	public getTerrain(
		...
		const geometry = new N3JS.PlaneGeometry(
			planeWidth,
			planeDepth,
			this.worldWidth - 1,
			this.worldDepth - 1
    ...
    */
	...
	public getMinecraft(
		...
		const planeGeometry = new N3JS.PlaneGeometry(planeWidth, planeDepth);
		const geometry: {
			px: I3JS.BufferGeometry;
			nx: I3JS.BufferGeometry;
			py: I3JS.BufferGeometry;
			pz: I3JS.BufferGeometry;
			nz: I3JS.BufferGeometry;
		} = {
			px: this.getGeometry(
				planeGeometry,
				[1, 3],
		...
		return N3JS.GeometryUtils.mergeBufferGeometries(geometries);
	}
	...
	public getMinecraftAo(
		...
		const planeGeometry = new N3JS.PlaneGeometry(
			planeWidth,
			planeDepth
		).toNonIndexed();
		...
		const worldHalfWidth = worldWidth / 2;
		const worldHalfDepth = worldDepth / 2;
		const matrix: I3JS.Matrix4 = new N3JS.Matrix4();
		for (let z = 0; z < worldDepth; z++) {
			for (let x = 0; x < worldWidth; x++) {
				const h = this.getY(x, z);
				matrix.makeTranslation(
	 ...
	 */
	public getGeometry(
		planeGeometry: I3JS.BufferGeometry,
		uv: number[],
	...
	public getTexture(
		...
		const vector3 = new N3JS.Vector3(0, 0, 0);
		sun.normalize();
		...
		for (let i = 0, j = 0, l = imageData.length; i < l; i += 4, j++) {
			...
			vector3.y = 2;
			vector3.z =
				this.data[j - this.worldWidth * 2] - this.data[j + this.worldWidth * 2];
			vector3.normalize();
			shade = vector3.dot(sun);
			imageData[i] = Math.min(
				255,
				(r + shade * r_add) * (0.5 + this.data[j] * 0.007)
			);
			imageData[i + 1] = Math.min(
				255,
				(g + shade * g_add) * (0.5 + this.data[j] * 0.007)
			);
			imageData[i + 2] = Math.min(
				255,
				(b + shade * b_add) * (0.5 + this.data[j] * 0.007)
			);
		}
