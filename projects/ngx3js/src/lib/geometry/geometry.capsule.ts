 ...
 *
 * See the [ngx3js docs](https://outmindkjg.github.io/ngx3js-doc/#/docs/ngxapi/en/CapsuleGeometry) page for details.
		...
		for (let iy = 0; iy <= vRadiusSegments; iy++) {
			...
			if (v < 0.5) {
				for (let ix = 0; ix <= radiusSegments; ix++) {
					...
					vertex.z =
						radius * Math.sin(phiStart + u * phiLength) * Math.sin(v * Math.PI);
