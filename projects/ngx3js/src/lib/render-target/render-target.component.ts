...
export class RenderTargetComponent extends AbstractTextureComponent implements OnInit, OnDestroy {
  ...
  private getTargetSize(width?: number, height?: number): THREE.Vector2 {
    ...
    return ThreeUtil.getVector2Safe(ThreeUtil.getTypeSafe(width, this.size, 1024), ThreeUtil.getTypeSafe(height, this.size, 1024), null, null, true).multiplyScalar(window.devicePixelRatio);
  }
  ...
  private getTargetTextureOptions(): THREE.WebGLRenderTargetOptions {
    ...
    if (ThreeUtil.isNotNull(this.depthTexture)) {
      ...
      options.depthTexture = new THREE.DepthTexture(targetSize.x, targetSize.y, ThreeUtil.getTextureDataTypeSafe(this.depthTexture.type), ThreeUtil.getMappingSafe(this.depthTexture.mapping), ThreeUtil.getWrappingSafe(this.depthTexture.wrapS, this.depthTexture.wrap), ThreeUtil.getWrappingSafe(this.depthTexture.wrapT, this.depthTexture.wrap), ThreeUtil.getTextureFilterSafe(this.depthTexture.magFilter, this.depthTexture.filter), ThreeUtil.getTextureFilterSafe(this.depthTexture.minFilter, this.depthTexture.filter), ThreeUtil.getTypeSafe(this.depthTexture.anisotropy));
    }
    return options;
  }
  /**
    ...
    if (this.renderTarget === null || this._needUpdate) {
      ...
      switch (this.type.toLowerCase()) {
        ...
        case 'webglcuberendertarget':
          ...
          this.renderTarget = new THREE.WebGLCubeRenderTarget(size, this.getTargetTextureOptions());
          break;
        case 'multiple':
        case 'multiplerender':
        case 'webglmultiplerender':
        case 'webglmultiplerendertargets':
          this.renderTarget = new THREE.WebGLMultipleRenderTargets(renderTargetSize.x, renderTargetSize.y, ThreeUtil.getTypeSafe(this.count, 1));
          break;
        case 'sample':
        case 'multisample':
        ...
        case 'webglmultisamplerendertarget':
          const webGLMultisampleRenderTarget = new THREE.WebGLMultisampleRenderTarget(renderTargetSize.x, renderTargetSize.y, this.getTargetTextureOptions());
        ...
        default:
          this.renderTarget = new THREE.WebGLRenderTarget(renderTargetSize.x, renderTargetSize.y, this.getTargetTextureOptions());
          break;
      }
      this.subscribeRefer(
        'targetSize',
