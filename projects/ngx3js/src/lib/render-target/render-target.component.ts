	...
	providers: [
		{
			...
			useExisting: forwardRef(() => RenderTargetComponent),
		},
	],
})
export class RenderTargetComponent
	...
	implements OnInit, OnDestroy
{
	 ...
	 */
	@Input() public targetSize: I3JS.IVector2 | SizeComponent = null;
	 ...
	 */
	private getTargetTextureOptions(): I3JS.IWebGLRenderTargetOptions {
		const options: I3JS.IWebGLRenderTargetOptions =
		...
		if (ThreeUtil.isNotNull(this.depthTexture)) {
			...
			options.depthTexture = new N3JS.DepthTexture(
				...
				ThreeUtil.getWrappingSafe(
					this.depthTexture.wrapS,
					this.depthTexture.wrap
				),
				ThreeUtil.getWrappingSafe(
					this.depthTexture.wrapT,
					this.depthTexture.wrap
				),
				ThreeUtil.getTextureFilterSafe(
					this.depthTexture.magFilter,
					this.depthTexture.filter
				),
				ThreeUtil.getTextureFilterSafe(
					this.depthTexture.minFilter,
					this.depthTexture.filter
				),
