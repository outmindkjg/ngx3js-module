...
export abstract class AbstractMaterialComponent extends AbstractSubscribeComponent implements OnInit, OnChanges, AfterContentInit, OnDestroy {
   ...
   */
  @Input() public onBeforeCompile: (shader: THREE.Shader, renderer?: THREE.WebGLRenderer) => void = null;
   ...
   */
  protected MATERIAL_ATTR: string[] = ['blending', 'blenddst', 'blenddstalpha', 'blendequation', 'blendequationalpha', 'blendsrc', 'blendsrcalpha', 'clipintersection', 'clippingplanes', 'clipshadows', 'colorwrite', 'defines', 'depthfunc', 'depthtest', 'depthwrite', 'fog', 'opacity', 'polygonoffset', 'polygonoffsetfactor', 'polygonoffsetunits', 'precision', 'premultipliedalpha', 'dithering', 'flatshading', 'shadowside', 'tonemapped', 'transparent', 'stencilwrite', 'stencilfunc', 'stencilref', 'stencilwritemask', 'stencilfuncmask', 'stencilfail', 'stencilzfail', 'stencilzpass', 'userdata', 'alphatest', 'name', 'side', 'vertexcolors', 'visible', 'texture', 'color', 'colormultiply'];
              ...
              default:
                ...
                if (mesh instanceof THREE.Scene) {
                  switch (materialType.toLowerCase()) {
                    ...
                    case 'background':
                      const backgroundTexture: THREE.Texture = materialAny['map'];
                    ...
                    case 'environment':
                      const environmentTexture: THREE.Texture = materialAny['map'];
                      if (ThreeUtil.isNotNull(environmentTexture)) {
                        mesh.environment = environmentTexture;
                      } else {
                        const environmentTextureEnv: THREE.Texture = materialAny['envMap'];
                    ...
                    case 'environmentbackgroundangular':
                      const bgEnvironmentTexture: THREE.Texture = materialAny['map'];
                      if (ThreeUtil.isNotNull(bgEnvironmentTexture)) {
                        mesh.environment = bgEnvironmentTexture;
                        mesh.background = bgEnvironmentTexture;
                      } else {
                        const bgEnvironmentTextureEnv: THREE.Texture = materialAny['envMap'];
