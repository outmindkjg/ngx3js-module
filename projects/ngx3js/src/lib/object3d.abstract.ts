...
export abstract class AbstractObject3dComponent extends AbstractTweenComponent implements OnInit, OnChanges, AfterContentInit, OnDestroy {
   ...
   * </p>
   */
  @Input() private onBeforeRender: (renderer?: THREE.WebGLRenderer, scene?: THREE.Scene, camera?: THREE.Camera, geometry?: THREE.BufferGeometry, material?: THREE.Material, group?: THREE.Group) => void = null;

  /**
   * The geometry of mesh
   */
  @Input() public geometry: AbstractGeometryComponent | THREE.BufferGeometry | any = null;
   ...
   */
  @ContentChildren(AbstractGeometryComponent, { descendants: false }) protected geometryList: QueryList<AbstractGeometryComponent>;
   ...
   * @returns materials
   */
  protected getMaterials(parameters?: THREE.MeshBasicMaterialParameters, required: boolean = true): THREE.Material | THREE.Material[] {
   ...
   */
  protected getMaterialOne(parameters?: THREE.MeshBasicMaterialParameters, required: boolean = true): THREE.Material {
   ...
   */
  protected getMaterialsMulti(parameters?: THREE.MeshBasicMaterialParameters, required: boolean = true): THREE.Material[] {
    const materials = this.getMaterials(parameters, required);
    if (Array.isArray(materials)) {
      return materials;
  ...
  protected applyChanges3d(changes: string[]) {
    if (this.object3d !== null) {
      ...
      if (ThreeUtil.isIndexOf(changes, ['init'])) {
        changes = ThreeUtil.pushUniq(changes, ['position', 'rotation', 'scale', 'lookat', 'visible', 'name', 'matrixautoupdate', 'layers', 'castshadow', 'receiveshadow', 'frustumculled', 'renderorder', 'lodistance', 'animationgroup', 'object3d', 'onbeforerender', 'rigidbody', 'controller', 'prefab', 'geometry', 'material', 'mixer']);
      }
